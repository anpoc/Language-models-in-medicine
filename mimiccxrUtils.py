'''
Script taken from: https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iv-cxr/txt/
'''
import re
...
def section_text(text, regex, section0='preamble'):
    remove_spacing = lambda x: re.sub('\s+', ' ', re.sub('^\s+|\\n|\s+$', '', x))
    section_regex = re.compile(regex, re.DOTALL)
    sections, section_names, section_idx = [], [], []

    current_section = section0
    previous_end = 0
    section = section_regex.search(text, previous_end)
    while section:
        current_end = section.start()
        # Saving values
        sections.append(remove_spacing(text[previous_end:current_end]))
        section_names.append(current_section)
        section_idx.append(previous_end)
        # Updating values
        current_section = section.group(1).lower().strip()
        previous_end = section.end()
        section = section_regex.search(text, previous_end)
    # Saving values
    sections.append(remove_spacing(text[previous_end:len(text)]))
    section_names.append(current_section if previous_end != 0 else 'full report')
    section_idx.append(previous_end)
